...
create local temp table n_mnth_inact
on commit preserve rows as
select
    nm.DataInfo,
...
where DataInfo >= (select max(DataInfo) from DataMart.DshECom_NomenclatureAnlsMnth_Inact)
...
cl33 as (
    ...
    from DataPrime.ECom_InactiveCL33Mnth
    where DataInfo >= (select max(DataInfo) from DataMart.DshECom_NomenclatureAnlsMnth_Inact)
    group by 1, 2, 3),
-- cl81 as (
--     select
...
select
    ...
    sum(active) + isnull(sum(inactive), 0) as TotalCards,
...
delete from DataMart.DshECom_NomenclatureAnlsMnth_Inact
...
select purge_table('DataMart.DshECom_NomenclatureAnlsMnth_Inact');

-- вставка в витрину с расшивкой 33 на уровни
insert into DataMart.DshECom_NomenclatureAnlsMnth_Inact
select
    DataInfo,
...
select analyze_statistics('DataMart.DshECom_NomenclatureAnlsMnth_Inact');
