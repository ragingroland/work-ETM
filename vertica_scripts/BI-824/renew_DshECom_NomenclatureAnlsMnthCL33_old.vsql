...
create local temp table n_mnth33
...
where DataInfo >= (select max(DataInfo) from DataMart.DshECom_NomenclatureAnlsMnthCL33)
...
select
    n_mnth33.DataInfo,
    n_mnth33.Class33_Code,
    n_mnth33.Status,
    count(distinct RgdCode) + isnull(Qlt_Inactive33, 0) as TotalCards
from n_mnth33
left join DataPrime.ECom_InactiveCL33Mnth nm33 on n_mnth33.DataInfo = nm33.DataInfo
    and n_mnth33.Status = nm33.Status
    and n_mnth33.Class33_Code = nm33.Class33_Code
group by 1, 2, 3, Qlt_Inactive33;

create local temp table activecards -- карточки с просмотрами
on commit preserve rows as
select
    DataInfo,
    Class33_Code,
...
delete from DataMart.DshECom_NomenclatureAnlsMnthCL33
...
select purge_table('DataMart.DshECom_NomenclatureAnlsMnthCL33');

-- вставка в витрину с расшивкой 33 на уровни
insert into DataMart.DshECom_NomenclatureAnlsMnthCL33
select
    DataInfo,
...
select analyze_statistics('DataMart.DshECom_NomenclatureAnlsMnthCL33');
