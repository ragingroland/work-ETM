...
group by trunc(cbs.DataInfo, 'q')::date, 2, 3, 4, 5, 6, 7, 8, 9
union -- объединяются
select
    trunc(cbs.DataInfo, 'q')::date as DataInfo,
    Class14_Code_2 as Class14_Code,
...
truncate table DataMart.DshMCB_FedRepQrt_TrnOvrTerr;
select purge_table('DataMart.DshMCB_FedRepQrt_TrnOvrTerr');
insert into DataMart.DshMCB_FedRepQrt_TrnOvrTerr
select * from tmp_local;
commit;

select analyze_statistics('DataMart.DshMCB_FedRepQrt_TrnOvrTerr');
