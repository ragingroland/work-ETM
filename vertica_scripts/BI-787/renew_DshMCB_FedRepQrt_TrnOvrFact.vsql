...
notE as ( -- исключения для 79Е
    ...
    group by 1),
main as ( -- ЭТМ-электрика
    select
        trunc(cbs.DataInfo, 'q')::date as DataInfo,
        cbs.CustInn,
        Class14_Code_2 as Class14_Code,
    ...
    where cbs.Class295_Code = '00001' -- считаем суммы только для ЭТМ
...
select -- делается итоговый селект нужных столбцов
    coalesce(main.DataInfo, main2.DataInfo) as a1,
    coalesce(main.CustInn, main2.CustInn) as a2,
    coalesce(main.Class14_Code, main2.Class14_Code) as a3,
    coalesce(main.Class63_Code_UP, main2.Class63_Code_UP) as a4,
    coalesce(main.Class63_Code_IG, main2.Class63_Code_IG) as a5,
    coalesce(main.Class294_Code, main2.Class294_Code) as a6,
    coalesce(main.Class295_Code, main2.Class295_Code) as a7,
    coalesce(main.Class296_Code, main2.Class296_Code) as a8,
    coalesce(main.Code_QuasiBrand, main2.Code_QuasiBrand) as a9,
    coalesce(main.Class79_Code, main2.Class79_Code) as a10,
    coalesce(main.SumShipped, main2.SumShipped) as a11
from main2
left join main on
    main.DataInfo = main2.DataInfo
    and main.CustInn = main2.CustInn
    and main.Class14_Code = main2.Class14_Code
    and main.Class63_Code_UP = main2.Class63_Code_UP
    and main.Class63_Code_IG = main2.Class63_Code_IG
    and main.Class294_Code = '03'
    and main.Class295_Code = main2.Class295_Code
    and main.Class296_Code = main2.Class296_Code
    and main.Code_QuasiBrand = main2.Code_QuasiBrand
    and main.Class79_Code = main2.Class79_Code
    and main.SumShipped = main2.SumShipped;

truncate table DataMart.DshMCB_FedRepQrt_TrnOvrFact;
select purge_table('DataMart.DshMCB_FedRepQrt_TrnOvrFact');
insert into DataMart.DshMCB_FedRepQrt_TrnOvrFact
select * from tmp_local;
commit;

truncate table DataMart.DshMCB_FedRepQrt_TrnOvrFact_QB;
select purge_table('DataMart.DshMCB_FedRepQrt_TrnOvrFact_QB');
insert into DataMart.DshMCB_FedRepQrt_TrnOvrFact_QB
select
    DataInfo,
    CustInn,
    Class63_Code_UP,
    ...
    Class79_Code,
    sum(SumShipped)
from DataMart.DshMCB_FedRepQrt_TrnOvrFact
group by 1, 2, 3, 4, 5, 6;
commit;

select analyze_statistics('DataMart.DshMCB_FedRepQrt_TrnOvrFact');
select analyze_statistics('DataMart.DshMCB_FedRepQrt_TrnOvrFact_QB');
