...
insert into slice
select
    isnull(max(DataInfo), add_months(trunc(current_date, 'mm'), -13)) as martdate -- если даты в витрине нет (по какой-то причине),
...
where CountryID in (19, 24) and (select martdate from slice) <= DataInfo;
----------------------------------------
...
-------------------------------------------
-- нужно удалить из витрины даты меньшие или равные минимальной дате во временной таблице
