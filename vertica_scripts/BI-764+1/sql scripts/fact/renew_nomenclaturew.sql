...
copy tmp_local (
    ...
    QltRefusal,
    RenameYM)
from local '/autons/vertica/web_analitik/nomenklature_week.csv'
    abort on error
    delimiter '|'
    null ''
    enclosed by '"'
    rejectmax 1
rejected data '/autons/vertica/web_analitik_run/nomenklature_week.rej'
exceptions '/autons/vertica/web_analitik_run/nomenklature_week.exc' direct skip 1;
...
delete from DataPrime.ECom_NomenclatureWk
where DataInfo >= (select mindate from dates) and DataInfo <= (select maxdate from dates);
commit;

insert into DataPrime.ECom_NomenclatureWk
select
    DataInfo,
    CounterID,
    RgdCode,
