...
create local temp table tmp_DshSalesManag_LongTerm_TotDivSpecInnMnth_fact ( -- врем.таблица для факта го
    DataInfo date,
    CustInn varchar(24),
...
truncate table tmp_DshSalesManag_LongTerm_TotDivSpecInnMnth_fact;

create local temp table tmp_DshSalesManag_LongTerm_TotDivSpecInnMnth_dsc -- врем.таблица для дсц
on commit preserve rows as
    select * from tmp_DshSalesManag_LongTerm_TotDivSpecInnMnth_fact;
truncate table tmp_DshSalesManag_LongTerm_TotDivSpecInnMnth_dsc;

create local temp table tmp_DshSalesManag_LongTerm_TotDivSpecInnMnth_7972 ( -- врем.таблица для 79 и 72
    CliInn varchar(24),
    MaxRang int,
    Class79_Code varchar(4))
on commit preserve rows 
order by CliInn;
truncate table tmp_DshSalesManag_LongTerm_TotDivSpecInnMnth_7972;

create local temp table tmp_DshSalesManag_LongTerm_TotDivSpecInnMnth_3763 ( -- врем.таблица для 37 и 63
    CliInn varchar(24),
    DataInfo date,
    Class63_Code_2 varchar(8),
    Class63_Code_3 varchar(12),
...
on commit preserve rows 
order by CliInn;
truncate table tmp_DshSalesManag_LongTerm_TotDivSpecInnMnth_3763;

create local temp table tmp_DshSalesManag_LongTerm_TotDivSpecInnMnth_merge -- врем.таблица для слияния факта и дсц
on commit preserve rows as
    select * from tmp_DshSalesManag_LongTerm_TotDivSpecInnMnth_fact;
truncate table tmp_DshSalesManag_LongTerm_TotDivSpecInnMnth_merge;

insert into tmp_DshSalesManag_LongTerm_TotDivSpecInnMnth_7972
select distinct
    glepsfc.CliInn,
...
insert into tmp_DshSalesManag_LongTerm_TotDivSpecInnMnth_3763
select distinct
    glepsfc.CliInn,
    klc37.DataInfo,
...
insert into tmp_DshSalesManag_LongTerm_TotDivSpecInnMnth_fact -- наполнение таблицы для факта
select
    cbs.DataInfo,
    cbs.CustInn,
    cla2.Class63_Code_2,
...
left join tmp_DshSalesManag_LongTerm_TotDivSpecInnMnth_7972 cla on cbs.CustInn = cla.CliInn
inner join Class72 cl72 on cla.MaxRang = cl72.RangValue
left join tmp_DshSalesManag_LongTerm_TotDivSpecInnMnth_3763 cla2 on cbs.CustInn = cla2.CliInn
...
group by
    cbs.DataInfo, cbs.CustInn, Class63_Code_2, Class63_Code_3, SalesChannel_, SalesSubChannel_,
    Class79_Code_, Class72_Code_;
commit;

insert into tmp_DshSalesManag_LongTerm_TotDivSpecInnMnth_dsc -- наполнение таблицы для дсц
with
    vw_ltgm as (
        select
...
from vw_ltgm
left join tmp_DshSalesManag_LongTerm_TotDivSpecInnMnth_7972 cla on vw_ltgm.CliInn = cla.CliInn
inner join Class72 cl72 on cla.MaxRang = cl72.RangValue
left join tmp_DshSalesManag_LongTerm_TotDivSpecInnMnth_3763 cla2 on vw_ltgm.CliInn = cla2.CliInn
...
group by 
    vw_ltgm.DataInfo, vw_ltgm.CliInn, Class63_Code_2, Class63_Code_3, SalesSubChannel_,
    Class79_Code_, SalesChannel_, Class72_Code_;
commit;

insert into tmp_DshSalesManag_LongTerm_TotDivSpecInnMnth_merge -- слияние врем.таблиц выше
select
...
from tmp_DshSalesManag_LongTerm_TotDivSpecInnMnth_fact fact
full join tmp_DshSalesManag_LongTerm_TotDivSpecInnMnth_dsc dsc
...
delete from tmp_DshSalesManag_LongTerm_TotDivSpecInnMnth_merge
where DataInfo < '2024-01-01'::date;
commit;

truncate table DataMart.DshSalesManag_LongTerm_TotDivSpecInnMnth;
insert into DataMart.DshSalesManag_LongTerm_TotDivSpecInnMnth
select
    DataInfo,
    CustInn,
    ...
    SumShipped,
    LongTermGoalMnth
from tmp_DshSalesManag_LongTerm_TotDivSpecInnMnth_merge;
commit;