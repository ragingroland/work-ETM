...
create local temp table tmp_DshSalesManag_LongTerm_DivSpecMnth_fact -- врем.таблица для факта го
on commit preserve rows as
    select * from DataMart.DshSalesManag_LongTerm_DivSpecMnth
where DshSalesManag_LongTerm_DivSpecMnth.DataInfo = '0001-01-01';
truncate table tmp_DshSalesManag_LongTerm_DivSpecMnth_fact;

create local temp table tmp_DshSalesManag_LongTerm_DivSpecMnth_dsc -- врем.таблица для дсц
on commit preserve rows as
    select * from DataMart.DshSalesManag_LongTerm_DivSpecMnth
where DshSalesManag_LongTerm_DivSpecMnth.DataInfo = '0001-01-01';
truncate table tmp_DshSalesManag_LongTerm_DivSpecMnth_dsc;

create local temp table tmp_DshSalesManag_LongTerm_DivSpecMnth_7972 ( -- врем.таблица для 79 и 72
    CliInn varchar(24),
    MaxRang int,
    Class79_Code varchar(4))
on commit preserve rows 
order by CliInn;
truncate table tmp_DshSalesManag_LongTerm_DivSpecMnth_7972;

create local temp table tmp_DshSalesManag_LongTerm_DivSpecMnth_3763 ( -- врем.таблица для 37 и 63
    CliInn varchar(24),
    DataInfo date,
...
on commit preserve rows 
order by CliInn;
truncate table tmp_DshSalesManag_LongTerm_DivSpecMnth_3763;

create local temp table tmp_DshSalesManag_LongTerm_DivSpecMnth_merge -- врем.таблица для слияния факта и дсц
on commit preserve rows as
    select * from DataMart.DshSalesManag_LongTerm_DivSpecMnth
where DshSalesManag_LongTerm_DivSpecMnth.DataInfo = '0001-01-01';
truncate table tmp_DshSalesManag_LongTerm_DivSpecMnth_merge;


insert into tmp_DshSalesManag_LongTerm_DivSpecMnth_7972
select distinct 
    glepsfc.CliInn,
...
insert into tmp_DshSalesManag_LongTerm_DivSpecMnth_3763
select distinct 
    glepsfc.CliInn,
    klc37.DataInfo,
...
insert into tmp_DshSalesManag_LongTerm_DivSpecMnth_fact -- наполнение таблицы для факта
select
    cbs.DataInfo,
    cla2.Class63_Code_2,
    cla2.Class63_Code_3,
...
left join tmp_DshSalesManag_LongTerm_DivSpecMnth_7972 cla on cbs.CustInn = cla.CliInn
inner join Class72 cl72 on cla.MaxRang = cl72.RangValue
left join tmp_DshSalesManag_LongTerm_DivSpecMnth_3763 cla2 on cbs.CustInn = cla2.CliInn
    and cbs.DataInfo = cla2.DataInfo
where
    cbs.class295_Code = '00001'
    and cbs.DataInfo >= ADD_MONTHS(trunc(CURRENT_DATE - 1 , 'YEAR'), -12)
group by 
    cbs.DataInfo, Class63_Code_2, Class63_Code_3, Class294_Code_, SalesSubChannel_,
    Class79_Code_, Class72_Code_;
commit; 

insert into tmp_DshSalesManag_LongTerm_DivSpecMnth_dsc -- наполнение таблицы для дсц
with vw_ltgm as (
    select
...
from vw_ltgm
left join tmp_DshSalesManag_LongTerm_DivSpecMnth_7972 cla on vw_ltgm.CliInn = cla.CliInn
inner join Class72 cl72 on cla.MaxRang = cl72.RangValue
left join tmp_DshSalesManag_LongTerm_DivSpecMnth_3763 cla2 on vw_ltgm.CliInn = cla2.CliInn
    and vw_ltgm.DataInfo = cla2.DataInfo
where
    vw_ltgm.DataInfo >= ADD_MONTHS(trunc(CURRENT_DATE - 1 , 'YEAR'), -12)
group by 
    vw_ltgm.DataInfo, Class63_Code_2, Class63_Code_3, Class294_Code_, SalesSubChannel_,
    Class79_Code_, Class72_Code_;
commit; 
   
insert into tmp_DshSalesManag_LongTerm_DivSpecMnth_merge -- слияние врем.таблиц выше
select
...
from tmp_DshSalesManag_LongTerm_DivSpecMnth_fact fact
full join tmp_DshSalesManag_LongTerm_DivSpecMnth_dsc dsc
...
delete from tmp_DshSalesManag_LongTerm_DivSpecMnth_merge
where DataInfo < '2024-01-01'::date;
commit;

truncate table DataMart.DshSalesManag_LongTerm_DivSpecMnth;
insert into DataMart.DshSalesManag_LongTerm_DivSpecMnth
select * from tmp_DshSalesManag_LongTerm_DivSpecMnth_merge;
commit;

drop table tmp_DshSalesManag_LongTerm_DivSpecMnth_fact;
drop table tmp_DshSalesManag_LongTerm_DivSpecMnth_dsc;
drop table tmp_DshSalesManag_LongTerm_DivSpecMnth_merge;
drop table tmp_DshSalesManag_LongTerm_DivSpecMnth_7972;
drop table tmp_DshSalesManag_LongTerm_DivSpecMnth_3763;