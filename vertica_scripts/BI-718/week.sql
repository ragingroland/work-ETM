...
select
    ...
    comp_status,
    '0' as outlier,
    avg(coalesce(PriceETM_NDS, 0)) over(partition by date_trunc('week', DataInfo), pcr.RgdCode) as PriceETM_NDS,
    min(coalesce(Comp_Price, 0)) over(partition by DataInfo, pcr.RgdCode, company_name, region_id, NetNum) as min_Comp_Price,
    avg(min(coalesce(Comp_Price, 0))) over(partition by date_trunc('week', DataInfo), pcr.RgdCode) as avg_Comp_Price
...
select
    ...
    '1' as outlier,
    avg(coalesce(PriceETM_NDS, 0)) over(partition by date_trunc('week', DataInfo), por.RgdCode) as PriceETM_NDS,
    min(coalesce(Comp_Price, 0)) over(partition by DataInfo, por.RgdCode, por.company_name, region_id, por.NetNum) as min_Comp_Price,
    avg(min(coalesce(Comp_Price, 0))) over(partition by date_trunc('week', DataInfo), por.RgdCode) as avg_Comp_Price
...
select
	*
from post
where datainfo >= '2024-07-15' and datainfo < '2024-07-22'
...
select
	*
from DataMart.DshPricing_ClustMinPrc_WklyAvg
where datainfo >= '2024-07-15' and datainfo < '2024-07-22'
