-- витрина для анализа по срезам "месяц"
...
insert into tmp_otdd (
    select
        ...
        DataInfo || ' ' || CliCode || ' ' || UsrInvNumReqDelivNum as compose_key,
...
insert into tmp_tcl (
    select
        ...
        case when delta is not null then tcl.LateCode else null end as LateCode
...
create local temp table tmp_DshLogistManag_OnTimeDeliv_Mnth (
    DataInfo date,
    MnthStrt date,
    StoreCode int,
    ReqNum varchar(150),
    ...
    LateCode int)
on commit preserve rows;
truncate table tmp_DshLogistManag_OnTimeDeliv_Mnth;

insert into tmp_DshLogistManag_OnTimeDeliv_Mnth (
select
    main.DataInfo,
    main.MnthStrt,
...
truncate table DataMart.DshLogistManag_OnTimeDeliv_Mnth;
insert into DataMart.DshLogistManag_OnTimeDeliv_Mnth
select 
    MnthStrt,
    StoreCode,
    ReqNum,
    ...
    ReqID,
    LateCode 
from tmp_DshLogistManag_OnTimeDeliv_Mnth;
commit;